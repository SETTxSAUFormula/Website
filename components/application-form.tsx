    ...
    <form onSubmit={handleSubmit} className="border border-white/15 bg-[#071b14] p-6 sm:p-8 lg:p-10" aria-busy={status === 'sending'}>
      <div className="space-y-10">
        ...
        <FormSection number="04" title={sectionTitles[3]}>
          ...
          <Field><FieldLabel htmlFor="application-motivation-factor" className={labelClass}>{content.motivationFactor}</FieldLabel><NativeSelect id="application-motivation-factor" name="motivationFactor" required defaultValue="" className={selectClass}><NativeSelectOption value="" disabled>{content.select}</NativeSelectOption><NativeSelectOption value="production">{language === 'tr' ? 'Somut bir araç ve ürün ortaya çıkarmak' : 'Building a tangible vehicle and product'}</NativeSelectOption><NativeSelectOption value="learning">{language === 'tr' ? 'Teknik ve kişisel olarak gelişmek' : 'Developing technical and personal skills'}</NativeSelectOption><NativeSelectOption value="competition">{language === 'tr' ? 'Yarışma ortamında yer almak' : 'Taking part in competitions'}</NativeSelectOption><NativeSelectOption value="teamwork">{language === 'tr' ? 'Disiplinler arası bir ekiple çalışmak' : 'Working in a multidisciplinary team'}</NativeSelectOption><NativeSelectOption value="career">{language === 'tr' ? 'Kariyer ve profesyonel çevre kazanmak' : 'Building a career and professional network'}</NativeSelectOption><NativeSelectOption value="other">{language === 'tr' ? 'Diğer' : 'Other'}</NativeSelectOption></NativeSelect></Field>
