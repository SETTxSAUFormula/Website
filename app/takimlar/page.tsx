...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: '/takimlar',
    languages: { 'tr-TR': '/takimlar', 'en-US': '/en/takimlar' },
  },
};
...
const pageCopy = {
  tr: {
    ...
    title: 'Takımımız',
    intro:
      'Takım liderinden departman şeflerine, ardından her departmanın üyelerine uzanan güncel organizasyon yapımız.',
  ...
  en: {
    ...
    title: 'Our Team',
    intro:
      'Our current organisation, from the team leader to department leads and every member within each technical department.',
...
function PersonCard({
  ...
  return (
    <article
      className={`group overflow-hidden border border-white/15 bg-[#071b14] ${featured ? 'grid sm:grid-cols-[0.8fr_1.2fr]' : ''}`}
    >
      <div
        className={`relative grid place-items-center overflow-hidden bg-white/[0.035] ${featured ? 'min-h-72' : 'aspect-[4/3]'}`}
      >
        <div
          className="absolute inset-x-0 bottom-0 h-px bg-racing-green/30"
          aria-hidden="true"
        />
        ...
        <span className="absolute bottom-5 text-[9px] font-bold uppercase tracking-[0.2em] text-white/25">
          {photoLabel}
        </span>
      </div>
      <div
        className={`flex flex-col justify-end ${featured ? 'p-8 sm:p-10' : 'p-5'}`}
      >
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-racing-green">
          {role}
        </p>
        <h3
          className={`mt-3 font-heading font-black uppercase ${featured ? 'text-4xl sm:text-5xl' : 'text-2xl'}`}
        >
          {name}
        </h3>
      </div>
  ...
  return (
    <main>
      ...
      <div className="relative isolate">
        <section className="bg-ink px-5 pb-16 pt-12 text-white lg:px-10 lg:pb-24 lg:pt-16">
          <div className="mx-auto max-w-[1500px]">
            <header className="grid gap-7 border-b border-white/15 pb-9 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-racing-green">
                  {copy.season}
                </p>
                <h1 className="mt-4 font-heading text-[clamp(3.8rem,7vw,6.8rem)] font-black uppercase leading-[0.88]">
                  {copy.title}
                </h1>
              </div>
            ...
            <section className="mt-12" aria-labelledby="team-leader-title">
              <div className="mb-5 border-b border-white/12 pb-4">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-racing-green">
                  01 / {copy.leaderKicker}
                </p>
                <h2
                  id="team-leader-title"
                  className="mt-2 font-heading text-4xl font-black uppercase"
                >
                  {copy.leaderTitle}
                </h2>
              </div>
              <div className="max-w-3xl">
                <PersonCard
                  name={copy.fullName}
                  role={copy.teamLeader}
                  photoLabel={copy.memberPhoto}
                  featured
                />
              </div>
            </section>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {copy.departments.map((department) => (
                  <PersonCard
                    key={department}
                    name={copy.fullName}
                    role={`${department} · ${copy.departmentLead}`}
                    photoLabel={copy.memberPhoto}
                  />
                ))}
              </div>
            ...
            >
              <div className="flex flex-col justify-between gap-5 border-b border-white/12 pb-6 lg:flex-row lg:items-end">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-racing-green">
                    03 / {copy.membersKicker}
                  </p>
                  <h2
                    id="department-members-title"
                    className="mt-2 font-heading text-4xl font-black uppercase sm:text-5xl"
                  >
                    {copy.membersTitle}
                  </h2>
                </div>
              ...
              <div className="mt-10 space-y-14">
                {copy.departments.map((department, departmentIndex) => (
                  <section
                    key={department}
                    aria-labelledby={`department-${departmentIndex}`}
                  >
                    <div className="flex items-center gap-4">
                      <span className="font-heading text-2xl font-black text-racing-green">
                        0{departmentIndex + 1}
                      </span>
                      <h3
                      ...
                      <span
                        className="h-px flex-1 bg-white/12"
                        aria-hidden="true"
                      />
                    </div>
        ...
        <div className="absolute inset-0 z-20 bg-ink/35 px-5 backdrop-blur-[3px]">
          <div className="sticky top-0 grid min-h-[calc(100svh-5rem)] place-items-center py-12">
            <div className="max-w-xl border border-racing-green/45 bg-[#03110d]/90 px-7 py-8 text-center shadow-2xl shadow-black/40 sm:px-10">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-racing-green">
                {copy.maintenanceKicker}
              </p>
              <p className="mt-3 font-heading text-3xl font-black uppercase text-white sm:text-4xl">
